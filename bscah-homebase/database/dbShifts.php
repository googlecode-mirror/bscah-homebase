<?php
    ...
    function selectDateVenue_dbShifts($date, $venue) {
     ...
     function select_persons_by_venue($venue) {
        connect();
        $query = "SELECT PERSONS FROM SHIFT WHERE VENUE =\"" . $venue . "\"" . " AND PERSONS IS NOT NULL";
        error_log('in select_persons_by_venue query is '.$query);
        $result = mysql_query($query);
        mysql_close();
        $persons = [];
        $persons_info = [];
...
// now see if it can be moved by looking at all other shifts for the same date and venue
        ...
        for ($i = 0; $i < mysql_num_rows($current_shifts); ++$i) {
            ...
            if (timeslots_overlap($same_day_shift[1], $same_day_shift[2], $new_s->get_start_time(),
                                  $new_s->get_end_time())) {
                $s = $old_s;
                mysql_close();
...
//Removed repeated code in line 349 - GIOVI
    ...
    function get_all_people_in_past_shifts() {
        ...
        foreach ($all_shifts as $a_shift) {
            if (substr($a_shift->get_id(), 6, 2) >= substr($today, 6, 2) &&
                substr($a_shift->get_id(), 0, 5) >= substr($today, 0, 5)
            ) {
                continue;
            } // skip present and future shifts
    ...
    function get_all_peoples_histories_in_shifts() { // This function is now returning the correct data - GIOVI
        ...
        $all_shifts = get_all_shifts();
        foreach ($all_shifts as $a_shift) {
            $persons = explode('*', $a_shift->get_persons());//The star is what's used to separate people's ids in the persons column in the shift table - GIOVI
            if (!$persons[0])  // skip vacant shifts
            {
                array_shift($persons);
            }
            if (count($persons) > 0) {
                foreach ($persons as $a_person) {
                    if (strpos($a_person, '+') > 0) {//The plus is used to state the end of a person's id, first name, and last name and should be placed before each star - GIOVI
                       
                        $person_id = substr($a_person, 0, strpos($a_person, '+'));
                        
                        if (array_key_exists($person_id, $histories)) { $histories[$person_id] .= "," . $a_shift->get_id(); }
                        
                        else { $histories[$person_id] = $a_shift->get_id(); }                                                    
                    }
                }
            }
        }
