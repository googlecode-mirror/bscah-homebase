<?php
    ...
    if ($id == 'new') {
        ...
        //problem?
        $person =
            new Person('new', 'applicant', null, null, null, null, null, null, null, null, null, null, null, null, null,
                       null, md5("new"), null);
...
<?PHP
    ...
    if ($_POST['_form_submit'] != 1) //in this case, the form has not been submitted, so show it
    {
        ...
        if ($errors) {
            ...
            $person =
                new Person($_POST['first_name'], $_POST['last_name'], $birthday, $_POST['gender'], $_POST['address'],
                           $_POST['city'], $_POST['state'], $_POST['zip'], $_POST['phone1'], $_POST['phone2'],
                           $_POST['email'],
                           $_POST['type'], $_POST['status'], $_POST['schedule'], $_POST['notes'], $_POST['old_pass'],
                           $ima,
    ...
    function process_form($id) {
        ...
        // try to reset the person's password
            ...
            if ($_POST['reset_pass'] == "RESET") {
                ...
                $newperson = new Person($first_name, $last_name, $birthday, $gender, $address, $city, $state, $zip,
                                        $clean_phone1, $clean_phone2, $email,
            ...
            // try to add a new person to the database
                ...
                if ($_POST['old_id'] == 'new') {
                    ...
                    if ($dup) {
                        ...
                        //edited by James Loeffler
                        $newperson =
                            new Person($first_name, $last_name, $birthday, $gender, $address, $city, $state, $zip,
                                       $clean_phone1, $clean_phone2, $email,
                                       $type, $status, $schedule,
                                       $notes, md5($pass), $availability, $contact_preference);
                        $result = add_person($newperson);
