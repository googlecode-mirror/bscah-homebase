...
define("NameFirst", "John");         //A person or shift from the
...
function addPersontoShift() //This will add a person to the persons column in the shift table; They will be added as personID+NameFirst+NameLast+
{
    ...
    echo "The shift's person is $addper2shift";
...
{
    $allhistories = get_all_peoples_histories(); // This returns a key sorted list of everyone's names that are or were in shifts; 
    ...
    report_by_volunteer_names($nameinarr, $allhistories, '', '');//This method takes an array of name(s) and the histories and should display the days for each individual and the number of hours they work for each day with a total - GIOVI
    
    report_volunteer_hours_by_day($allhistories, '', ''); //This method takes all the histories from everyone and displays the total hours everyone works for each day and timeshift as well as the total - GIOVI
