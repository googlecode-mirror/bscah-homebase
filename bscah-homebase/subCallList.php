...
<?php
    ...
    function generate_scl($id) {
        ...
        connect();
        $query =
            "SELECT * FROM dbPersons WHERE (status = 'active' AND type LIKE '%sub%' AND availability LIKE '%" . $day .
            ":" . $time . "%') ORDER BY last_name,first_name";
...
//			}
        ...
        add_log_entry('<a href=\"personEdit.php?id=' . $_SESSION['_id'] . '\">' . $_SESSION['f_name'] . ' ' .
                      $_SESSION['l_name'] . '</a> generated a <a href=\"subCallList.php?shift=' . $shift->get_id() .
                      '\">sub call list</a> for the shift: <a href=\"editShift.php?shift=' .
                      $shift->get_id() . '&venue=' . $venue . '\">' . get_shift_name_from_id($shift->get_id()) .
                      '</a>.');
        //$_POST['_shiftid']=$id;
    ...
    function view_scl($id) {
        ...
        echo "<table width=\"700\" align=\"center\" border=\"1px\">
				<tr><td colspan=\"5\" align=\"center\"><b>Sub Call List for " . get_shift_name_from_id($id) .
            "</b></td></tr>";
        ...
        */
        ...
        for ($i = 0; $i < count($persons); ++$i) {
            if ($_SESSION['access_level'] >= 2) {
                echo "<tr><td>&nbsp;<a href=\"personEdit.php?id=" . $persons[$i][0] . "\">" . $persons[$i][1] . " " .
                    $persons[$i][2] . "</a></td><td>";
            }
            else {
    ...
    function process_edit_scl($post) {
        ...
        for ($i = 0; $i < count($persons_old); ++$i) {
            $p_new = [$persons_old[$i][0], $persons_old[$i][1], $persons_old[$i][2], $persons_old[$i][3],
                $persons_old[$i][4], trim(str_replace(',', '&#44;', str_replace('+', '&#43;', str_replace('\'', '\\\'',
                                                                                                          htmlentities($post[
                                                                                                                       'datecalled_' .
                                                                                                                       $i]))))),
                trim(str_replace(',', '&#44;', str_replace('+', '&#43;', str_replace('\'', '\\\'',
                                                                                     htmlentities($post['notes_' .
                                                                                     $i]))))),
                $post['accepted_' . $i]];
            $persons_new[] = $p_new;
        ...
        if ($new_acceptances > $vacancies) {
            for ($j = 0; $j < count($accepted_people); ++$j) {
                if ($j == 0) {
                    ...
                    if ($j == count($accepted_people) - 1) {
                        $s = $s . " and " . $persons_new[$accepted_people[$j]][1] . " " .
                            $persons_new[$accepted_people[$j]][2];
                    }
                    else {
                        $s = $s . ", " . $persons_new[$accepted_people[$j]][1] . " " .
                            $persons_new[$accepted_people[$j]][2];
                    }
                }
            ...
            for ($j = 0; $j < count($accepted_people); ++$j) {
                add_log_entry('<a href=\"personEdit.php?id=' . $_SESSION['_id'] . '\">' .
                              ...
                              '</a> assigned <a href=\"personEdit.php?id=' . $persons_new[$accepted_people[$j]][0] .
                              '\">' .
                              $persons_new[$accepted_people[$j]][1] . ' ' . $persons_new[$accepted_people[$j]][2] .
                              '</a> to the shift: <a href=\"editShift.php?shift=' .
                              $shift->get_id() . '&venue=' . $venue . '\">' . get_shift_name_from_id($shift->get_id()) .
                              '</a>.');
            }
            //print_r($shift);
